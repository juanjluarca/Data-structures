...
class MinHeap {
  ...
  private _deleteSwap(node: TreeNode){
    ...
    if (node.left !== null && node.left.data.price < smallest.data.price) {  
      smallest = node.left;
    }

    if (node.right !== null && node.right.data.price < smallest.data.price) {  
      smallest = node.right;
    }
  ...
  public preorder(ref: TreeNode | null = this.root): string {
    ...
    if (ref.left === null && ref.right === null) {
      return `Compañía: ${ref.data.name} Acciones: ${ref.data.stocks} Precio: ${ref.data.price}`;
    }
    
    let result = `Compañía: ${ref.data.name} Acciones: ${ref.data.stocks} Precio: ${ref.data.price}\n`;
  ...
  public findRight(ref: TreeNode | null = this.root): TreeNode | null {
    if (ref === null) {
      return null;
    }
